...
function install_cabal_layer () {
	expect_vars HALCYON_DIR HALCYON_REBUILD_CABAL HALCYON_UPDATE_CABAL HALCYON_NO_BUILD

	local tag source_dir
	expect_args tag source_dir -- "$@"

	if ! (( HALCYON_REBUILD_CABAL )) && ! (( HALCYON_UPDATE_CABAL )) &&
		restore_updated_cabal_layer "${tag}"
	then
		return 0
	fi

	if ! (( HALCYON_REBUILD_CABAL )) && restore_bare_cabal_layer "${tag}"; then
